...
export function parseAnswerDimensions(answer) {
  ...
  // Person comes in two formats: explicit "<first|second|third> person" (the
  // canonical form auto-generated by paradigm_morphology.js) and the
  // abbreviated "<1st|2nd|3rd|first|second|third> <sg|pl|singular|plural>"
  // used in hand-written grammar.js cards ("1st singular ('I am')"). The
  // lookahead requires a number qualifier so "1st aorist" / "2nd declension"
  // don't false-positive as person markers.
  const personWordMap = { '1st': 'first', '2nd': 'second', '3rd': 'third', first: 'first', second: 'second', third: 'third' };
  ...
  if (!person) {
    ...
    if (abbrev) person = personWordMap[abbrev[1]] || '';
  }
