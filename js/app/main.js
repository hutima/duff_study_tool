...
function buildCumulativeConfirmationSeries(cards, marksStore, options = {}) {
  ...
  const progressRecords = useWeightedProgress
    ? (cards || []).map(card => {
        const progress = getWordProgress(card.id);
        return {
      ...
      }).filter(record => record.ts && record.contribution > 0)
    : cards.map(card => ({ ts: getWordProgress(card.id).firstConfirmedAt || 0, contribution: 1 })).filter(record => record.ts);

  const currentConfirmed = useWeightedProgress
    ? (cards || []).reduce((sum, card) => sum + getProgressAverageContribution(card, marksStore), 0)
    : (cards || []).filter(card => marksStore?.[card.id] === 'known').length;

  if (!progressRecords.length) return { total, currentConfirmed, weeklyPct: 0, series: [] };
  progressRecords.sort((a, b) => a.ts - b.ts);
  const dailyAdds = {};
  progressRecords.forEach(record => { const key = getUsageDayKey(record.ts); dailyAdds[key] = (dailyAdds[key] || 0) + record.contribution; });
  ...
  const recentCount = progressRecords
...
function buildCertaintyBuckets(cards, marksStore, options = {}) {
  ...
  (cards || []).forEach(card => { buckets[getCertaintyBucketForCard(card, marksStore, options)] += 1; });
  return buckets;
}
